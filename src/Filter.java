/**
 * A sound filter to normalize played sounds.
 * 
 * @author Joshua Barry
 * 
 */
public final class Filter {
	...
	private float interpolateGain(int i, int j, float f) {
		float ratio = (float) anIntArrayArrayArray667[i][0][j] + f
				* (float) (anIntArrayArrayArray667[i][1][j] - anIntArrayArrayArray667[i][0][j]);
		ratio *= 0.001525879F;
		return 1.0F - (float) Math.pow(10D, -ratio / 20F);
	}

	/**
	 * the frequency for the C note is 32.7032 Hz octave is the number of
	 * octaves above note C our note is
	 * 
	 * @param octaves
	 * @return
	 */
	...
	private float poleFrequency(float f, int i, int j) {
		float f1 = (float) anIntArrayArrayArray666[j][0][i] + f
				* (float) (anIntArrayArrayArray666[j][1][i] - anIntArrayArrayArray666[j][0][i]);
		f1 *= 0.0001220703F;
		return normalize(f1);
	}
	...
	public int method544(int i, float f) {
		...
		if (anIntArray665[i] == 0)
			return 0;
		float f2 = interpolateGain(i, 0, f);
		aFloatArrayArray669[i][0] = -2F * f2 * (float) Math.cos(poleFrequency(f, 0, i));
		aFloatArrayArray669[i][1] = f2 * f2;
		for (int k = 1; k < anIntArray665[i]; k++) {
			float f3 = interpolateGain(i, k, f);
			float f4 = -2F * f3 * (float) Math.cos(poleFrequency(f, k, i));
			float f5 = f3 * f3;
	...
	public void method545(RSBuffer stream, Envelope class29) {
